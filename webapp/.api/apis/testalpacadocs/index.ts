...
class SDK {
  ...
  constructor() {
    ...
    this.core = new APICore(this.spec, 'testalpacadocs/1.1 (api/6.1.3)');
  }
   ...
   * > ⚠️ Warning
   * >
   * > Currently Alpaca has no guarantees on the creation time of corporate actions. There
   * may be delays in receiving corporate actions from our data providers, and there may be
   * delays in processing and making them available via this API. As a result, corporate
   * actions may not be available immediately after they are announced.
   *
   ...
   * between the specified dates.
   * The oldest date to retrieve historical quotes of us-1 location is 14th October, 2025
   * 12AM UTC.
   *
   ...
   * you are likely to see only one symbol in your first response if there are enough bars
   * for that symbol to hit the limit you requested.
   *
   * In these situations, if you keep requesting again with the `next_page_token` from the
   * previous response, you will eventually reach the other symbols if any bars were found
   * for them.
   ...
   * the specified dates.
   *
   * The returned results are sorted by symbol first, then by the quote timestamp. This means
   * that you are likely to see only one symbol in your first response if there are enough
   * quotes for that symbol to hit the limit you requested.
   *
   * In these situations, if you keep requesting again with the `next_page_token` from the
  ...
  /**
   * The latest quotes endpoint provides the latest best bid and ask prices for the given
   * ticker symbols.
   *
   * @summary Latest quotes
   ...
   * the specified dates.
   *
   * The returned results are sorted by symbol first then by trade timestamp. This means that
   * you are likely to see only one symbol in your first response if there are enough trades
   * for that symbol to hit the limit you requested.
   *
   * In these situations, if you keep requesting again with the `next_page_token` from the
   * previous response, you will eventually reach the other symbols if any trades were found
   ...
   *
   * Trades with any conditions that causes them to not update the bar price are excluded.
   * For example a trade with condition `I` (odd lot) will never appear on this endpoint. You
   * can find the complete list of excluded conditions in [this
   * FAQ](https://docs.alpaca.markets/docs/market-data-faq#how-are-bars-aggregated).
   *
   * @summary Latest trades
  ...
  /**
   * The latest quote endpoint provides the latest best bid and ask prices for a given ticker
   * symbol.
   *
